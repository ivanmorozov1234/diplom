...
def my_program():
    prof = current_user.profile
    if not prof or not prof.assigned_program_id:
        flash(
            "Сначала заполните профиль и выберите цель — система подберёт программу по полю «Тип цели» у программы в админке.",
            "warning",
        )
        return redirect(url_for("main.profile"))
